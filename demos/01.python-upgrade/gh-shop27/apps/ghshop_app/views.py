...
def process(request):
    if request.method == "POST":
        ...
        if request.POST['product_id'] == "book":
            ...
            request.session['total_spent'] += request.session['price']
            request.session['total_purchases'] += amount
    return redirect('/ghshop/checkout', request)

def checkout(request):
    context = {
        'price': request.session['price'],
        'total_spent': request.session['total_spent'],
        'total_purchases': request.session['total_purchases']
    }
    print request.session['price'], request.session['total_spent'], request.session['total_purchases']
